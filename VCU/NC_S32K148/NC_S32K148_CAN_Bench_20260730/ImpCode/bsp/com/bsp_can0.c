...
#if ((VCU_ENABLE_BENCH_DEBUG != 0U) && (VCU_BENCH_SINGLE_CAN0 != 0U))
static bool CAN0_ProcessStdFrame(const can_message_t *message)
{
	...
	{
		case 0x13U:
			memcpy(can0_recv1.ESP_T_03, message->data, 8U);
			can0_recv1.esp_t03_rx_count++;
			break;
		case 0x16U:
			memcpy(can0_recv1.ESP_T_06, message->data, 8U);
			can0_recv1.esp_t06_rx_count++;
			break;
		case 0x17U:
			memcpy(can0_recv1.ESP_T_07, message->data, 8U);
			can0_recv1.esp_t07_rx_count++;
			break;
		case 0x1E4U:
			...
			memcpy(can0_recv1.res_msg, message->data, 8U);
			can0_recv1.res_rx_count++;
			break;
		case 0x11U:
			memcpy(can0_recv1.canb_ipctsmsg1, message->data, 8U);
			can0_recv1.ipc_msg1_rx_count++;
			break;
		case 0x12U:
			memcpy(can0_recv1.canb_ipctsmsg2, message->data, 8U);
			can0_recv1.ipc_msg2_rx_count++;
			break;
		case 0x42U:
			memcpy(can0_recv1.AMI_Tx, message->data, 8U);
			can0_recv1.ami_rx_count++;
			break;
		case 0x401U:
			memcpy(can0_recv1.canb_epssendmsg1, message->data, 8U);
			can0_recv1.eps_msg1_rx_count++;
			break;
		case 0x402U:
			memcpy(can0_recv1.canb_epssendmsg2, message->data, 8U);
			can0_recv1.eps_msg2_rx_count++;
			break;
		case VCU_DBG_CONTROL_ID:
			memcpy(can0_recv1.vcu_dbg_control, message->data, 8U);
			can0_recv1.vcu_dbg_control_rx_count++;
			break;
		case VCU_DBG_PEDAL_ADC_ID:
			memcpy(can0_recv1.vcu_dbg_pedal_adc, message->data, 8U);
			can0_recv1.vcu_dbg_pedal_rx_count++;
			break;
		case VCU_DBG_AIR_WHEEL_ID:
			memcpy(can0_recv1.vcu_dbg_air_wheel, message->data, 8U);
			can0_recv1.vcu_dbg_air_wheel_rx_count++;
			break;
		case VCU_DBG_DIGITAL_ID:
			memcpy(can0_recv1.vcu_dbg_digital, message->data, 8U);
			can0_recv1.vcu_dbg_digital_rx_count++;
			break;
		default:
			return false;
	}
...
#if ((VCU_ENABLE_BENCH_DEBUG != 0U) && (VCU_BENCH_SINGLE_CAN0 != 0U))
				notifyTask = CAN0_ProcessStdFrame(&recvMsg_CAN0_STD);
#endif
