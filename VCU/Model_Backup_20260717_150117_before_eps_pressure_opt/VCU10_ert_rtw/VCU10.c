...
void VCU10_step(void)
{
   ...
   *  Gain: '<S14>/Gain'
   */
  airprs_sensor1 = 40960.0 * (real_T)airprs1 * 0.000244140625;

  /* DataTypeConversion: '<S14>/Data Type Conversion15' incorporates:
   *  Gain: '<S14>/Gain1'
   */
  airprs_sensor2_tmp = 40960.0 * (real_T)airprs2 * 0.000244140625;
  ...
  rtb_BitwiseAND_mj = (uint8_T)rtb_mode;

  /* Outputs for Atomic SubSystem: '<S146>/Bit Shift6' */
  VCU10_BitShift3_m((uint8_T)rtb_mode, &rtb_BitwiseAND_mj);
   ...
   */
  rtb_mode = ((canb_epssendmsg1[4] << 8) + canb_epssendmsg1[5]) - 1024;
   ...
   */
  VCU10_BitShift_p((uint16_T)canb_epssendmsg2[2], &rtb_Add2_g);
   ...
   */
  VCU10_DW.steer_zero = ((rtb_mode >= -2) && (rtb_mode <= 2));
  ...
  rtb_mode = ((VCU10_DW.clockTickCounter_a2 < 50) &&
  ...
  rtb_mode = ((VCU10_DW.clockTickCounter < 50) && (VCU10_DW.clockTickCounter >=
    0));
  ...
  rtb_mode = ((VCU10_DW.clockTickCounter_a < 50) && (VCU10_DW.clockTickCounter_a
    >= 0));
  ...
  if (!(VCU10_B.pedal_error != 0.0)) {
    airprs_sensor1 = (apps1_01 + apps2_01) / 2.0 * 50.0;
  } else {
    airprs_sensor1 = 0.0;
  }
    ...
    switch (VCU10_DW.is_c4_VCU10) {
     ...
     case VCU10_IN_Default_m:
      ...
      if (VCU10_B.manual_driving_flag_k == 1.0) {
        ...
        /* Sum: '<S169>/Add4' */
        VCU10_B.motor_torque = airprs_sensor1;
      } else if (asms) {
         ...
         *  Inport: '<Root>/canb_ipctsmsg1'
         */
        VCU10_B.motor_torque = ((real_T)canb_ipctsmsg1[6] +
          rtb_DataTypeConversion1) - 5000.0;

        /* End of Outputs for S-Function (fcgen): '<Root>/10ms11' */

        /* Saturate: '<S237>/Saturation' */
        if (VCU10_B.motor_torque > 55.0) {
          /* Sum: '<S169>/Add4' */
          VCU10_B.motor_torque = 55.0;
        } else if (VCU10_B.motor_torque < 0.0) {
          /* Sum: '<S169>/Add4' */
          VCU10_B.motor_torque = 0.0;
        }
  ...
  tmp = floor(VCU10_B.motor_torque);
  if (rtIsNaN(tmp) || rtIsInf(tmp)) {
    tmp = 0.0;
  } else {
    tmp = fmod(tmp, 65536.0);
   ...
   *  DataTypeConversion: '<S238>/Data Type Conversion7'
   */
  rtb_y_cq = (uint16_T)((tmp < 0.0 ? (int32_T)(uint16_T)-(int16_T)(uint16_T)-tmp
    : (int32_T)(uint16_T)tmp) + 5000U);
