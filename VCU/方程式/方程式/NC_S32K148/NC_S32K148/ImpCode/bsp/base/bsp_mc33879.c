...
void MC33879_Control(void)
{
	...
	vTaskDelay(1);
	control_cmd = 0xFF00+MC33879.cmd1;
	SPI_MasterTransfer(&spi2_mc338Instance, &control_cmd, &mc338_recv[0], 1);
	vTaskDelay(1);
	MC33879.state1 = mc338_recv[0];

    //cmdH控制芯片2。位控制，高位控制LO8-LO5 低位控制HO8-HO5
	//eg: 0x10 控制LO5 0x01控制HO5
	SPI_SetSS(&spi2_mc338Instance, LPSPI_PCS1);
	vTaskDelay(1);
	control_cmd = 0xFF00+MC33879.cmd2;
	SPI_MasterTransfer(&spi2_mc338Instance, &control_cmd, &mc338_recv[1], 1);
	vTaskDelay(1);
	MC33879.state2 = mc338_recv[1];
}
