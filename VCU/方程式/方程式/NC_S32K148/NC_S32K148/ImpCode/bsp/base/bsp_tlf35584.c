...
static void Gp_Tlf35584_TransmitData(uint8_t len)
{
	...
	for(i=0; i<len; i++)
	{
		SPI_MasterTransfer(&spi0_tlf35Instance, &Tlf35584_Data.TxBuf[i], &Tlf35584_Data.RxBuf[i], 1);
		vTaskDelay(1);
	}
	vTaskDelay(1);
}
...
{
	//ERR 引脚输出。PWM 12KHZ duty50%
	PWM_Init(&pwm6_tlf35Instance, &pwm6_tlf35Configs);
