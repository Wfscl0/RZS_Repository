...
- 当前模型仍使用待标定查表：约0.252 V对应0，约2.000 V对应1；这只是当前占位标定，不代表最终传感器标定。

三个通道均为0.252 V时，原始帧示例：

```text
68 00 68 00 68 00 00 00
```

### 气压与轮速（0x7A2）

- `Dbg_AirPressure1/2`：直接填写MPa，范围0～1 MPa；对应0～10 V气压传感器。
- `Dbg_WheelFront_Hz/Dbg_WheelRear_Hz`：填写轮速传感器频率，单位Hz。
- 油压不在该调试帧中；四轮油压必须通过ESP反馈报文 `ESP_T_03(0x13)` 模拟。

两路气压0.6 MPa、前后轮速频率100 Hz时，原始帧示例：

```text
80 3E 80 3E E8 03 E8 03
```
...
| HIN4 | Dbg_HIN4_Cockpit | Cockpit DI8 |
| HIN5 | Dbg_HIN5_Side | Side DI8 |
