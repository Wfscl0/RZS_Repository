...
' the upper 13 mm of the main housing steps inward. Ten horizontal M4 screws
...
' Removing the cover counterbore and extending the neck gives 4.5 mm minimum
' nominal solid skirt below the long-side slots (4.7 mm on the short sides).
' With the specified +/-0.2 mm process allowance, the worst case remains >4 mm.
SIDE_SCREW_Z = 62
SIDE_BOSS_D = 11
SIDE_BOSS_DEPTH = 12
...
Function BuildMainHousing()
  ...
  ' The top opening passes through the full 13 mm neck, leaving a protected
  ...
  ' The outer battery-cover labyrinth groove is deliberately cut as the FINAL
  ' housing operation. Earlier versions cut it here, then later bottom-boss
  ' ribs added resin back across the groove and interrupted the water barrier.
  ...
  ' remaining 2.8 mm outer skin is continuous and waterproof; 4 mm full-wall
  ' ribs remain between adjacent cells. Lower corner cells are omitted around
  ' the shock-mount supports. A broad full-thickness keep-out surrounds the SMA
  ' sealing land and cable bend radius; no relief cell may approach that zone.
  ...
  For i = 0 To 6
    ...
    For j = 0 To 2
      ...
      If Not (y = 12 And Abs(x) > 55) Then
        If Not (Abs(x - SMA_X) < 30 And Abs(y - SMA_Z) < 28) Then
  ...
  ' left full thickness. Pockets never break the continuous 2.8 mm outer skin.
  ...
  ' Front-wall SMA-K bulkhead. R6 keeps the complete 4 mm wall around the
  ' connector: no D20 local thinning and no waffle pocket in the keep-out.
  ' This removes the crescent-shaped recess visible above the former cutout.
  ...
  ' FINAL continuous battery labyrinth cut. Performing this after every bottom
  ' boss and rib guarantees a single closed water path with no resin bridges.
  ' The groove remains outside the primary ID100 O-ring and leaves 2 mm of the
  ' original bottom wall, backed on the cavity side by the W7 x H1.5 ring.
  Set sk = NewSketch(body, refXY, "SK_BAT_OUTER_LABYRINTH_GROOVE_FINAL")
  Set f = sk.OpenEdition()
  ...
  sk.CloseEdition
  Set feat = AddPocket(part, sk, BAT_LAB_GROOVE_D, "POCKET_BAT_OUTER_LABYRINTH_CONTINUOUS_FINAL")

  AddNotes part, "Black high-toughness SLA final: supplier-table M4 x D5 x L8 inserts use D5.1 x 8.5 blind pockets. The ten upper side pockets are straight bores and receive a post-print 0.2-0.3 mm hand deburr/chamfer; this avoids coincident Pocket seams in the production STL. Their bosses use D24 x T3 horizontal double-arm ribs only, preventing any rib from crossing the Z68 cover/gasket interface; the eight bottom bosses retain four-way ribs and modeled D5.6 x 0.5 lead-ins. Install only after same-batch D4.9/D5.1/D5.3 coupon validation, using controlled low-temperature press plus toughened epoxy. The inset neck uses a 3 mm die-cut flat silicone gasket with a positive 2.3 mm compression stop. Ten side M4 x 8 screws remain outside the primary seal. The bottom labyrinth groove is a final continuous cut after every boss/rib. The SMA D10.4 bulkhead retains a full 4 mm wall and a broad solid no-relief zone. Other relieved wall cells retain a continuous 2.8 mm nominal waterproof skin and every load path remains full 4 mm."
  HideObject doc, hb
  part.Update
...
Sub WriteBuildParameters(stream)
  ...
  stream.WriteLine "SMA CUTOUT = FRONT WALL X" & SMA_X & " / Z" & SMA_Z & ", D" & SMA_HOLE_D & " mm, full T4 mm wall; no local thinning"
  stream.WriteLine "SMA SOLID KEEP-OUT = no waffle relief within the broad X42..BODY_EDGE / Z20..TOP_NECK region around the sealing land"
  stream.WriteLine "BATTERY LABYRINTH CONTINUITY = final housing cut after all bottom bosses and ribs; closed loop required"
