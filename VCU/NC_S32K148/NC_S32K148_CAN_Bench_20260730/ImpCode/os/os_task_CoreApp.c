...
static void Task_CoreApp_LoadCanRx(const HCAN_APP_RX_PAR *source);
...
static void Task_CoreApp_LoadCanRx(const HCAN_APP_RX_PAR *source)
{
	/* CAN回调队列传递的是静态缓存指针，复制期间需避免ISR更新该缓存。 */
	taskENTER_CRITICAL();
	memcpy(&hcan1_recv, source, sizeof(hcan1_recv));
	taskEXIT_CRITICAL();

	memcpy(ESP_T_03, hcan1_recv.ESP_T_03, 8U);
	memcpy(ESP_T_06, hcan1_recv.ESP_T_06, 8U);
	memcpy(ESP_T_07, hcan1_recv.ESP_T_07, 8U);
	memcpy(res_msg, hcan1_recv.res_msg, 8U);
	memcpy(canb_ipctsmsg1, hcan1_recv.canb_ipctsmsg1, 8U);
	memcpy(canb_ipctsmsg2, hcan1_recv.canb_ipctsmsg2, 8U);
	memcpy(AMI_Tx, hcan1_recv.AMI_Tx, 8U);
	memcpy(canb_epssendmsg1, hcan1_recv.canb_epssendmsg1, 8U);
	memcpy(canb_epssendmsg2, hcan1_recv.canb_epssendmsg2, 8U);
#if (VCU_ENABLE_BENCH_DEBUG != 0U)
	memcpy(vcu_dbg_control, hcan1_recv.vcu_dbg_control, 8U);
	memcpy(vcu_dbg_pedal_adc, hcan1_recv.vcu_dbg_pedal_adc, 8U);
	memcpy(vcu_dbg_air_wheel, hcan1_recv.vcu_dbg_air_wheel, 8U);
	memcpy(vcu_dbg_digital, hcan1_recv.vcu_dbg_digital, 8U);
...
static void Task_CoreApp_SignalReceive(void)
{
	...
	if(xQueueReceive(Que_Hcan0_CanIn, &(pcan0_recv), 0) == pdTRUE)
	{
		...
		Task_CoreApp_LoadCanRx(pcan0_recv);
#endif
	}
	...
	{
#if ((VCU_ENABLE_BENCH_DEBUG == 0U) || (VCU_BENCH_SINGLE_CAN0 == 0U))
		Task_CoreApp_LoadCanRx(pcan1_recv);
#endif
	}
