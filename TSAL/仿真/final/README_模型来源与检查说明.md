...
- 删除断线锁存区误放的重复 `R57`。现在 `R57` 只保留在信号来源/默认值区，`R46` 保留为 `V_DISCONNECT` 的另一处原理图 pull-up，`R47` 保留为 `REL_FAULT_SET` pull-up。
- 所有 `.asc` 中的活动 `.save ...` 指令已改成普通注释，不再限制保存波形节点。
- 在断线锁存功能块右侧增加了明显的“上电复位 POR”框，标出 `R64.1/U20.A=N1N224`、`C18.1/R55.2/R64.2=N1N223`、`U20.Y=N1N198 reset pulse`。
