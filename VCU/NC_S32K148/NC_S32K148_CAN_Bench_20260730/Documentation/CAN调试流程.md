...
1. AS_Emergency会在同一模型周期把TS和电机需求扭矩强制为0。台架需要使用CAN0，因此前期应断开逆变器CAN支路或可靠禁能逆变器，并断开高压，只观察报文。
2. `brake_on`主要由ESP四轮油压反馈判断；制动踏板电压连续约100 ms低于0.1 V或高于4.9 V会置位`brake_fault`，使`brake_on`和ASB安全许可失效。
...
| 通道 | 用途 | 波特率 | 帧类型 |
|---|---|---:|---|
| CAN0（本测试副本） | 电机扩展帧，以及ESP、EPS、RES、IPC、AMI和调试标准帧 | 500 kbit/s | 11位标准帧+29位扩展帧 |
| CAN1（本测试副本） | 本流程不使用 | 500 kbit/s | 保留 |

台架两端正确终端时，断电测量CAN_H与CAN_L之间应约为60 Ω。TSMaster只连接VCU CAN0；逆变器和其他实际执行器从该总线断开或可靠禁能。

本体工程仍保持“CAN0电机、CAN1其他节点”的原车分工；这里只描述`NC_S32K148_CAN_Bench_20260730`测试副本。
...
| `0x7A1` | APP1=0.252 V，APP2=0.252 V，Brake=0.5 V（零油门安全初值） | `40 05 40 05 6B 0A 00 00` |
...
若Requested=1而某组Active=0，说明该组数据帧没有被CAN0正确接收、DLC不是8，或该组已经超过500 ms未刷新。
...
初次测试保持逆变器CAN支路和高压断开，但TSMaster继续连接VCU CAN0。
...
EBS Emergency和RES Stop项只能在逆变器/执行器断开或可靠禁能时测试。Emergency时需求扭矩与CAN0电机使能位均应在同一模型周期清零。
