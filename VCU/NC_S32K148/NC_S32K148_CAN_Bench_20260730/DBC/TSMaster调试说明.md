...
- CAN0：500 kbit/s，台架副本在此通道同时承载电机扩展帧、IPC、RES、AMI、ESP、EPS以及调试标准帧。
- CAN1：本次单通道TSMaster调试不使用，仍保留硬件初始化。
- 均为 Classical CAN，DLC 8；调试报文使用11位标准帧。

将本DBC加载到TSMaster的CAN0通道。`0x7A0～0x7B2`只能用于测试副本，不要发送给生产本体。
...
供应商ESP数据库中的`ESP_T_01`也使用`0x11`，与当前IPC报文冲突。当前测试固件明确将CAN0标准帧`0x11`作为`IPC_Tx_Msg1`，因此整合DBC没有重复加入`ESP_T_01`。

## 7. 观察VCU输出

- CAN0扩展帧：`MCU1_Rx_Command`、`MCU1_Rx_Command3`。
- CAN0标准帧：`ESP_R_00(0x20)`、`AMI_Rx(0x41)`、`EPS_ReceiveMessage(0x469)`、`IPC_Rx_Msg(0x10)`以及`0x7B0～0x7B2`调试回显。
- 调试回显：`0x7B0～0x7B2`。
