...
- 最新 BOM：`bspd.xlsx`
- 原理图：`bspd.png`
- 分析优先级：**网表 > 原理图图片；器件型号和封装以原理图/BOM为准**
...
| 连接器 | 针脚 | 网络 | 类型 | 当前电气定义 |
|---|---:|---|---|---|
| H1 | 1 | `CURRENT` | 模拟输入 | 板端比较阈值约 1.202 V |
| H1 | 2 | `GND` | 电源回路 | 低压地 |
| H2 | 1 | `OIL4` | 模拟输入 | 板端比较阈值约 1.484 V |
| H2 | 2 | `OIL3` | 模拟输入 | 同上 |
| H2 | 3 | `OIL2` | 模拟输入 | 同上 |
| H2 | 4 | `OIL1` | 模拟输入 | 同上 |
...
- 原接口表中的“油压 0.75 V”和“电流 1.11 V”与当前网表分压不一致。当前板端阈值应分别按 R27/R28 和 R25/R26 计算为约 1.484 V、1.202 V。
- 上述值只代表 PCB 引脚电压阈值；对应油压和功率/电流物理量必须用实际传感器传递函数标定，不能直接由本板网表推出。
- `SC_IN`/`SC_OUT` 是继电器常开触点，不是 5 V 逻辑输出。掉电或驱动失效时触点断开，这是本设计的失效安全状态。
- 当前网表中 Q2 的源漏方向仍需按第 6.4 节进行实物封装与仿真双重核对。
...
| 信号 | 含义 | 低状态测试值 | 高状态测试值 |
|---|---|---:|---:|
| `OIL1` | 制动压力输入 1 | 0.5 V | 2.0 V |
| `OIL2` | 制动压力输入 2 | 0.5 V | 2.0 V |
| `OIL3` | 制动压力输入 3 | 0.5 V | 2.0 V |
| `OIL4` | 制动压力输入 4 | 0.5 V | 2.0 V |
| `CURRENT` | 功率/电流判定输入 | 0.5 V | 2.0 V |

以上测试电压仅用于验证比较器逻辑。最终必须根据实际压力传感器、电流传感器和 TS 电压重新建立传感器模型。
...
```text
CURRENT_VALID = 1，当 CURRENT > 1.2019 V
```
...
```text
Q2.2 → C3/R10/U7.6/U7.7 定时节点
Q2.3 → GND
```

若 AO3400 的物理引脚为 `2=Source、3=Drain`，则当前连接方向与标准低边放电接法相反。

Codex 必须生成两个版本：

```text
A. as_drawn_q2：严格按当前网表连接
B. corrected_q2：Q2.2 接 GND，Q2.3 接 C3 定时节点
```

必须同时绘制 C3 节点波形。  
若 `as_drawn_q2` 中体二极管钳位 C3，导致 C3 无法正常充电，则在报告中明确判定当前网表需要交换 Q2 的 2、3 引脚。

不得为了让仿真通过而只使用没有体二极管的理想开关模型。
...
在复位计时 5 s 时重新施加冲突 100 ms，然后再次清除。
...
6. AO3400 模型必须包含体二极管。
7. 严格建立 as_drawn_q2 和 corrected_q2 两种连接，比较 C3 定时节点。
8. 完成 T01～T14 全部测试。
